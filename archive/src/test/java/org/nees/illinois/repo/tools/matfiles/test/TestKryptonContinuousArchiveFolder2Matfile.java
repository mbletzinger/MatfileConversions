...
public class TestKryptonContinuousArchiveFolder2Matfile {
	...
	public void setUp() throws Exception {
		URL u = ClassLoader
				.getSystemResource("Krypton/CONT/Samples_3_HDR.txt");
		String df = u.getPath();
		for (int i = 0; i < 2; i++) {
	...
	public void testArchiveFolderProperties() {
		ArchiveFolderProperties afp = new KryptonContinuousArchiveFolder();
		afp.scan(dataFolder);
...
//			File path = new File(dataFolder);
	...
	public void testArchiveFolder2Matfile() {
		...
		try {
			new MatFileWriter("KryptonContTest.mat", af2m.getMlList());
		} catch (IOException e) {
