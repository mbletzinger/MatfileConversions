...
import org.nees.illinois.matfiles.archive.filetypes.OmVoltsArchiveFolder;
...
public class TestOmVoltsArchiveFolder2Matfile {
	private final Logger log = Logger.getLogger(TestArchive2Dto.class);
	private String dataFolder;

	@BeforeClass
	public void setUp() throws Exception {
		URL u = ClassLoader.getSystemResource("OM/Volts__headers.txt");
		String df = u.getPath();
	...
	public void testArchiveFolderProperties() {
		ArchiveFolderProperties afp = new OmVoltsArchiveFolder();
		afp.scan(dataFolder);
...
//			File path = new File(dataFolder);
			...
			Assert.assertEquals(2, dfiles.size());
	...
	public void testArchiveFolder2Matfile() {
		...
		try {
			new MatFileWriter("OmVoltsTest.mat", af2m.getMlList());
		} catch (IOException e) {
