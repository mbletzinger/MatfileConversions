...
public class OmContinuousArchiveFolder implements ArchiveFolderProperties {
	private final List<ArchiveFileProperties> pfiles = new ArrayList<ArchiveFileProperties>();

	public OmContinuousArchiveFolder() {
	}
	...
	public boolean isArchive(String folder) {
		...
		boolean found = false;
		for (String f : files) {
			if (f.contains(pfiles.get(0).getName() + "_Continuous")) {
				found = true;
			}
		}
		return found;
