...
import org.nees.illinois.matfiles.archive.filetypes.DaqContinuousArchive;
...
public class TestDaqContinuousArchiveFolder2Matfile {
	...
	public void setUp() throws Exception {
		URL u = ClassLoader
				.getSystemResource("DAQ/CONT/OM/LBCB2_Load_Actuator_HDR.txt");
		String df = u.getPath();
		for (int i = 0; i < 3; i++) {
	...
	public void testArchiveFolderProperties() {
		ArchiveFolderProperties afp = new DaqContinuousArchiveFolder();
		afp.scan(dataFolder);
...
//			File path = new File(dataFolder);
			...
			int esize = 3;
			if(((DaqContinuousArchive)f).getType().equals("OM")) {
				esize = 1;
			}
	...
	public void testArchiveFolder2Matfile() {
		...
		try {
			new MatFileWriter("DaqContTest.mat", af2m.getMlList());
		} catch (IOException e) {
